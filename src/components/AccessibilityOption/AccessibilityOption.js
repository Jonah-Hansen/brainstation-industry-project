...
function AccessibilityOption({ mini, type, title, description, icon, handleSwitch, handleColor, features, previewColor = '#5592F6' }) {
    ...
    <div className='option-row'>
      {type === 'colors' ?
        <>
          ...
          {mini ? <div className={`option-row__color`} style={{ backgroundColor: previewColor }} />
        ...
        <>
          ...
          <hgroup className='option-row__details'>
            <h2 className='option-row__title'>{title}</h2>
            {!mini && <p className='option-row__description'>{description}</p>}
          </hgroup>
